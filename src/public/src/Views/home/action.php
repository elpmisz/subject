...
if ($action === "login") {
  try {
    ...
    if (intval($count) === 0 || intval($verify) === 0) {
      $VALIDATION->alert("danger", "E-MAIL OR PASSWORD IS INCORRECT!", "/");
    }

    $status = $USER->user_status([$email]);
    if (intval($status) === 2) {
      $VALIDATION->alert("danger", "CONTACT ADMINISTRATOR!", "/");
    }
    ...
    $VALIDATION->alert("success", "LOGGED IN!", "/home");
...
if ($action === "logout") {
  try {
    $VALIDATION->logout();
    $VALIDATION->alert("success", "LOGGED OUT!", "/");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
